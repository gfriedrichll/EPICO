...
data <- read.csv("~/EPICO/Datos/España/EPICOCOVID19_DATA_2023-04-29_1325.csv")


#Arreglar base de datos - viejo dataset
#-------------------------------------------------------------------------------
#Setting Labels
...
freq(data$dsterm)
freq(data$excluded_reason)
freq(data$excluded_reason_ot)
data_españa <- data %>% filter(dsterm != "Excluded" | is.na(dsterm)==T) #Eliminar excluidos
data_españa <- data_españa %>% filter(dsterm != "Discharged alive (not hospitalized)" | is.na(dsterm)==T) #Eliminar no hospitalizados

data_españa$admission_date %<>% as.Date()
data_españa$discharge_date %<>% as.Date()
dsterm_NA <- data_españa %>% filter(is.na(dsterm)==T) %>% select(record,hospital,admission_date,discharge_date,hosp_time,dsterm)#Solo pacientes que no tenemos informacion para variable dsterm
summary(dsterm_NA)

data_españa_analisis <- data_españa %>% filter(is.na(dsterm)==F) #Eliminar pacientes con status dsterm NA
data_españa_sensibilidad <- data_españa  #Sensibilidad utilizando pacientes con dsterm NA

#Chequeo pacientes hospitalizados - minimo tiempo hospitalización es 2 dias 
data_españa_check_hosp <- data_españa %>% filter((hosp_time)>1) %>% select(hosp_time, dsterm)
summary(data_españa_check_hosp)
data_españa_check_hosp <- data_españa_analisis %>% filter((hosp_time)>1) %>% select(hosp_time, dsterm)
summary(data_españa_check_hosp)
#-------------------------------------------------------------------------------
...
summary(data$age_months) #167 unknown age


#Unknown age
data_españa_analisis$admission_date %<>% as.Date()
data_españa_analisis$discharge_date %<>% as.Date()
data_españa_analisis$patient_birth_date %<>% as.Date()

summary(data_españa_analisis$age_months) #13 unknown age

#Unknown age
data_españa_sensibilidad$admission_date %<>% as.Date()
data_españa_sensibilidad$discharge_date %<>% as.Date()
data_españa_sensibilidad$patient_birth_date %<>% as.Date()

summary(data_españa_sensibilidad$age_months) #102 unknown age

#Neonatos - total

data %<>% mutate(age_days=admission_date-patient_birth_date)
data %>% select(age_months,patient_birth_date, admission_date,age_days) %>% filter(abs(age_days) < 28) # 135 neonatos

#Neonatos - analisis

data_españa_analisis %<>% mutate(age_days=admission_date-patient_birth_date)
data_españa_analisis %>% select(age_months,patient_birth_date, admission_date,age_days) %>% filter(abs(age_days) < 28) # 134 neonatos

#Neonatos - sensibilidad

data_españa_sensibilidad %<>% mutate(age_days=admission_date-patient_birth_date)
data_españa_sensibilidad %>% select(age_months,patient_birth_date, admission_date,age_days) %>% filter(abs(age_days) < 28) # 135 neonatos

#Excluir neonatos
data_españa_analisis_sinneonatos <- data_españa_analisis %>% filter(abs(age_days) >= 28 | is.na(age_days)==T)
data_españa_sensibilidad_sinneonatos <- data_españa_sensibilidad %>% filter(abs(age_days) >= 28 | is.na(age_days)==T)

#Excluir adultos
data_españa_analisis_sinneonatos <- data_españa_analisis_sinneonatos %>% filter(abs(age_months) < 250 | is.na(age_months)==T)
data_españa_sensibilidad_sinneonatos <- data_españa_sensibilidad_sinneonatos %>% filter(abs(age_months) < 250 | is.na(age_months)==T)

#-------------------------------------------------------------------------------

#Cargar base de datos
data <- read.csv("~/EPICO/Datos/Colombia/EPICOCOLOMBIA_DATA_2023-01-27_1606.csv")
...
neonatos <- c("30656-16","30654-2","30653-51","30653-40","30422-158","30426-2","30426-26","30426-27","30426-38","30426-76","30426-82","30426-97","30653-11","30653-14",
              "30653-17","30653-24","30653-27","30653-28","30653-41","30653-64","30653-65","30655-1","30655-5","30655-8","30655-16","30420-44")

data=data[!data$record %in% neonatos,]
which(is.na(data$age_months))
data_colombia<-data[-125,]#No tiene info age
#-------------------------------------------------------------------------------

#Variables diferentes
#-------------------------------------------------------------------------------
setdiff(names(data_españa), names(data_colombia))
setdiff(names(data_colombia), names(data_españa))
names(data_colombia)[names(data_colombia) == "corticost_cmyn_date"] <- "costicost_date"
setdiff(names(data_colombia), names(data_españa))
#-------------------------------------------------------------------------------
...
data_españa_analisis$Country <- "España"
data_españa_analisis_sinneonatos$Country <- "España"
data_españa_sensibilidad$Country <- "España"
data_españa_sensibilidad_sinneonatos$Country <- "España"


#Union
datos_todos_analisis <- smartbind(data_colombia, data_españa_analisis, fill = 0)
datos_todos_sensibilidad <- smartbind(data_colombia, data_españa_sensibilidad, fill = 0)
datos_todos_analisis_sinneonatos <- smartbind(data_colombia, data_españa_analisis_sinneonatos, fill = 0)
datos_todos_sensibilidad_sinneonatos <- smartbind(data_colombia, data_españa_sensibilidad_sinneonatos, fill = 0)

#write.csv(datos_todos,"~/EPICO/Datos/V2_datos_todos_01272023.csv", row.names = TRUE)
#write.csv(datos_todos,"~/EPICO/Datos/datos_todos_01272023.csv", row.names = TRUE)
write.csv(datos_todos_analisis,"~/EPICO/Datos/datos_todos_analisis_9MAY2023.csv", row.names = TRUE)
write.csv(datos_todos_sensibilidad,"~/EPICO/Datos/datos_todos_sensibilidad_9MAY2023.csv", row.names = TRUE)
write.csv(datos_todos_analisis_sinneonatos,"~/EPICO/Datos/datos_todos_analisis_sinneonatos_9MAY2023.csv", row.names = TRUE)
write.csv(datos_todos_sensibilidad_sinneonatos,"~/EPICO/Datos/datos_todos_sensibilidad_sinneonatos_9MAY2023.csv", row.names = TRUE)
